...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposableSidePreview = vscode.commands.registerCommand('ssq.sidePreview', async () => {

        await preview.initMarkdownPreview(vscode.ViewColumn.Two);

    });

    let disposableStandalonePreview = vscode.commands.registerCommand('ssq.fullPreview', async () => {
